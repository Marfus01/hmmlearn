...
# 尝试导入C++扩展，如果失败则使用Python实现
try:
    from . import nested_hmmc
    _NESTED_HMMC_AVAILABLE = True
except ImportError:
    _NESTED_HMMC_AVAILABLE = False
    import warnings
    warnings.warn("nested_hmmc C++ extension not available, using slower Python implementation")
...
class NestedHMM(_AbstractHMM):
    ...
    def __init__(self, n_actors, n_iter=100, tol=1e-2, verbose=False,
        ...
        if implementation == "auto":
            self.implementation = "cpp" if _NESTED_HMMC_AVAILABLE else "python"
        elif implementation == "cpp" and not _NESTED_HMMC_AVAILABLE:
            raise ValueError("C++ implementation requested but nested_hmmc extension not available")
        else:
    ...
    def _do_forward_pass(self, X_1, X_2):
        ...
        """
        if self.implementation == "cpp" and _NESTED_HMMC_AVAILABLE:
            return self._do_forward_pass_cpp(X_1, X_2)
        else:
            return self._do_forward_pass_python(X_1, X_2)
    
    def _do_forward_pass_cpp(self, X_1, X_2):
        """
        前向算法的C++实现版本
        """
        if not _NESTED_HMMC_AVAILABLE:
            raise RuntimeError("C++ implementation not available")
        
        fwdlattice = nested_hmmc.nested_forward_pass(
            X_1.astype(np.int32),
            X_2.astype(np.int32),
            self.alpha_.astype(np.float64),
            self.A_F_.astype(np.float64),
            self.beta_.astype(np.float64),
            self.A_S_.astype(np.float64),
            self.B_F_.astype(np.float64),
            self.B_S_.astype(np.float64),
            float(self.gamma1_[0]),
            float(self.gamma2_[0]),
        )
        
        return fwdlattice
    
    def _do_forward_pass_python(self, X_1, X_2):
        """
        前向算法的Python实现版本（保持原有逻辑作为后备）
        """
        n_samples = len(X_1)
    ...
    def _do_backward_pass(self, X_1, X_2):
        ...
        """
        if self.implementation == "cpp" and _NESTED_HMMC_AVAILABLE:
            return self._do_backward_pass_cpp(X_1, X_2)
        else:
            return self._do_backward_pass_python(X_1, X_2)
    
    def _do_backward_pass_cpp(self, X_1, X_2):
        """
        后向算法的C++实现版本
        """
        if not _NESTED_HMMC_AVAILABLE:
        ...
        bwdlattice = nested_hmmc.nested_backward_pass(
            X_1.astype(np.int32),
            X_2.astype(np.int32),
            self.alpha_.astype(np.float64),
            self.A_F_.astype(np.float64),
    ...
    def _do_backward_pass_python(self, X_1, X_2):
        """
        后向算法的Python实现版本（保持原有逻辑作为后备）
        """
        n_samples = len(X_1)
        face_configs = self._enumerate_face_configs()
        n_face_configs = len(face_configs)
